...
/*   Updated: 2023/12/21 22:26:08 by ldon             ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
{
	static char	*join_content[1024] = {NULL};
	char		*line;

	if (BUFFER_SIZE <= 0 || fd < 0 || fd >= 1024)
		return (NULL);
	join_content[fd] = ft_read_line(fd, join_content[fd]);
	if (join_content[fd] == NULL)
		return (NULL);
	line = ft_extract_line(join_content[fd]);
	join_content[fd] = update_after_newline(join_content[fd]);
	return (line);
}
/*
#include <fcntl.h>
